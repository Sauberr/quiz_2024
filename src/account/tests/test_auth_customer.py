...
class TestAuthCustomer(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.admin.set_password("1234567890")
        self.admin.save()

    def test_user_login_wrong_email(self):
        user_login = self.client.login(email="wrong_email", password="1234567890")
    ...
    @unittest.skip("We do not have a index page.")
    ...
    def test_user_access_admin_panel_expected(self):
        self.client.force_login(self.user)
        response = self.client.get(reverse("admin:index"))
        self.assertEqual(response.status_code, HTTPStatus.OK)
